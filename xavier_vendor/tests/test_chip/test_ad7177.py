...
def ad7177():
    with mock.patch.object(Axi4Ad717x, "__init__", return_value=None) as mock_init:
        return AD7177("/dev/AXI4_DMM_AD7177",(5000,"mV"))


class TestAD7177Class():

    def test_init(self):
        with mock.patch.object(Axi4Ad717x, "__init__", return_value=None) as mock_init:
            vref = (5000,"mV")
            ad7177 =  AD7177("/dev/AXI4_DMM_AD7177",vref)
            assert ad7177.vref == vref

    def test_single_sample_mode(self,ad7177):
    ...
    def test_voltage_channel_enable(self,ad7177):
        ...
        for return_val in return_vals:
            for channel,config in args:
                with mock.patch.object(AD7177, "register_write", return_value=return_val) as mock_register_write:
                    ret = ad7177.voltage_channel_enable(channel,config)
                    assert ret==return_val
    ...
    def test_voltage_single_channel_select(self,ad7177):
        return_vals=[True,False]
        args=[("chan0",{"P":"AIN0","N":"AIN1"}),
                   ("chan1", {"P":"AIN2","N":"AIN3"}),
                   ("chan2", {"P":"AIN2","N":"AIN3"}),
        ...
        for return_val in return_vals:
            for channel,config in args:
                with mock.patch.object(AD7177, "register_write", return_value=return_val) as mock_register_write:
                    ret = ad7177.voltage_single_channel_select(channel,config)
                    assert ret==return_val
    ...
    def test_voltage_get(self,ad7177):
        return_vals=[(False,(0x00,0x5678),(3000,"mV"),False),
                                (True,False,(3000,"mV"),False),
                                (True,(0x00,0x5678),False,False),
                                ]
        for return_val in return_vals:
            with mock.patch.object(AD7177, "voltage_single_channel_select", return_value=return_val[0]) as mock_voltage_single_channel_select:
                with mock.patch.object(Axi4Ad717x, "single_sample_code_get", return_value=return_val[1]) as mock_single_sample_code_get:
                    with mock.patch.object(Axi4Ad717x, "code_2_mvolt", return_value=return_val[2]) as mock_code_2_mvolt:
                        ret = ad7177.voltage_get("chan0",{"P":"AIN0","N":"AIN1"})
        ...
        for arg in args:
            with mock.patch.object(AD7177, "voltage_single_channel_select", return_value=True) as mock_voltage_single_channel_select:
                with mock.patch.object(Axi4Ad717x, "single_sample_code_get", return_value=arg[0]) as mock_single_sample_code_get:
                    with mock.patch.object(Axi4Ad717x, "code_2_mvolt", return_value=(3000,"mV")) as mock_code_2_mvolt:
                        ret = ad7177.voltage_get(arg[1],arg[2])
                        assert ret==(3000,"mV")
        ...
        for code,channel,config in args:
            with mock.patch.object(AD7177, "voltage_single_channel_select", return_value=True) as mock_voltage_single_channel_select:
                with mock.patch.object(Axi4Ad717x, "single_sample_code_get", return_value=code) as mock_single_sample_code_get:
                    with mock.patch.object(Axi4Ad717x, "code_2_mvolt", return_value=(3000,"mV")) as mock_code_2_mvolt:
                        ret = ad7177.voltage_get(channel,config)
                        assert ret == False


    def test_is_comunication_ok(self,ad7177):
        args=[(0xffff,False),
                    (0x4FD0,True),
                    (0x0000,False),
                    (0x0FF0,False),
        ...
        for read_return,result_return in args:
            with mock.patch.object(AD7177, "register_read", return_value=read_return) as mock_register_read:
                ret = ad7177.is_comunication_ok()
                assert ret == result_return

    def test_ifmode_register_set(self,ad7177):
        test_args=[
                    ("enable","24bits",True),
                    ("disable","24bits",True),
                    ("enable","32bits",True),
                    ("disable","32bits",False)
                ]
        for data_stat_channel_flag, data_bits,result in test_args:
            with mock.patch.object(AD7177, "register_write", return_value=result) as mock_register_write:
                ret = ad7177.ifmode_register_set(data_stat_channel_flag,data_bits)
                assert ret == result
    ...
    def test_mode_register_set(self,ad7177):
        ...
        for clock_source,result in test_args:
            with mock.patch.object(AD7177, "register_write", return_value=result) as mock_register_write:
                ret = ad7177.mode_register_set(clock_source)
                assert ret == result

        with mock.patch.object(AD7177, "register_write", return_value=True) as mock_register_write:
            try:
                ret = ad7177.mode_register_set("test_error")
                assert False
            except KeyError:
                assert True

    def test_setup_register_set(self,ad7177):
        ...
        for return_val in return_vals:
            for channel,code_polar,reference,buffer_flag in test_args:
                with mock.patch.object(AD7177, "register_write", return_value=return_val) as mock_register_write:
                    ret = ad7177.setup_register_set(channel,code_polar,reference,buffer_flag )
                    assert ret == return_val
    ...
    def test_samplerate_set(self,ad7177):
        ...
        for read_return,write_return,result_return in return_vals:
            for channel,samplerate in test_args:
                with mock.patch.object(AD7177, "register_read", return_value=read_return) as mock_register_read:
                    with mock.patch.object(AD7177, "register_write", return_value=write_return) as mock_register_write:
                        ret = ad7177.samplerate_set(channel,samplerate)
                        assert ret == result_return
        ...
        for channel,samplerate in test_args:
            with mock.patch.object(AD7177, "register_read", return_value=0x500) as mock_register_read:
                with mock.patch.object(AD7177, "register_write", return_value=True) as mock_register_write:
                    try:
                        ret = ad7177.samplerate_set(channel,samplerate)
                        assert False
                        ...
                        assert True
    def test_samplerate_get(self,ad7177):
        ...
        for channel,return_val,result in test_args:
            with mock.patch.object(AD7177, "register_read", return_value=return_val) as mock_register_read:
                ret = ad7177.samplerate_get(channel)
                assert ret == result
    ...
    def test_continue_mode_frame_config_set(self,ad7177):
        ...
        for channel,samplerate,result in test_args:
            with mock.patch.object(Axi4Ad717x, "frame_channel_and_samplerate_set", return_value=result) as mock_frame_channel_and_samplerate_set:
                ret = ad7177.continue_mode_frame_config_set(channel,samplerate)
                assert ret == result

    def test_continue_sample_mode(self,ad7177):
        ...
        for result in test_args:
            with mock.patch.object(Axi4Ad717x, "continue_sample_mode", return_value=result) as mock_continue_sample_mode:
                ret = ad7177.continue_sample_mode()
                assert ret == result


    def test_continue_mode_codes_get(self,ad7177):
        ...
        for count,result in test_args:
            with mock.patch.object(Axi4Ad717x, "data_analysis", return_value=result) as mock_data_analysis:
                ret = ad7177.continue_mode_codes_get(count)
                assert ret == result