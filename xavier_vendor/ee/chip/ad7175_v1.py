...
import time
from ee.ipcore.axi4_ad717x_v1 import Axi4Ad717x
from ee.common import logger
from ee.chip.ad7175 import AD7175

__version__="1.1.0"

class AD7175_V1(AD7175):
    ...
    def voltage_get(self,channel,config):
        ...
        '''
        code = data[1]
        volt = self.code_2_mvolt(code,self.vref,24)
        return volt
...
#Continue mode
    ...
    def continue_mode_codes_get(self,data_count):
        ...
        """
        rd_data = self.device.continue_sample_capture()
#         logger.error("rd_data:%s" % (rd_data))
        get_data = []
        for i in range(0,data_count):
            offset = i*4
            rd_int = (rd_data[offset+1]) | (rd_data[offset+2] << 8) | (rd_data[offset+3] << 16)
