...
from ee.board.programmer.chipprog.psoc4prog import Psoc4Prog
from ee.board.programmer.chipprog.otpprog import OtpProg
from ee.board.programmer.chipprog.nrf5xbleprog import Nrf5XProg
from ee.board.programmer.chipprog.spiflashprog import SpiFlash
from ee.board.programmer.chipprog.stm32prog import Stm32Prog
#-----from python layer-----
from ee.board.programmer.chipprog.hearstprog import HearstProg
import time
...
class ProgApp():
    ...
    def __target_erase(self,target_address=None,size=None):
        ret_str = "erasing target ....\r\n"
        ret = self.prog_instance.dut_storage_erase(0,target_address,size)
        ret_str += _return_to_str(ret)
        if ret[0] == False:
            self.destroy_target()
            return False,ret_str
    ...
    def target_programming(self,target_file_name,size,target_address,file_offset = 0):
        ...
        ret_str += "target address is %08x"%(target_address)+".\r\n"
        ...
        ret = self.__target_erase()# ret = self.__target_erase(target_address,size)
        ret_str += ret[1] 
        if ret[0] == False:
            self.destroy_target()
            return False,ret_str
