...
from ee.board.b31forbp.ad5761 import AD5761
from ee.board.b31forbp.axi4_time_detect_v1 import Axi4TimeDetect_v1
import time
from ee.ipcore.axi4_aid_slave import Axi4AidSlave

from ee.ipcore.axi4_swd_core import Axi4SwdCore
from ee.ipcore.axi4_ds18b20 import Axi4Ds18b20

class B31FORBP(object):
...
#         self.dac_device['Base_Board']=AD5061(self.profile['Base_Board']['path'],self.profile['Base_Board']['vref'])   
 ...
 #       time.sleep(0.01)  
  ...
  #          if self.profile['dacfunction'][key]['partno']=="AD5761":
            if  self.profile['dacfunction'][key]['partno']=="AD5761":
                self.gpio_device_name.gpio_set(self.profile["dacfunction"][key]['gpio'])
            self.dac_device[key]=eval(self.profile['dacfunction'][key]['partno'])(self.profile['dacfunction'][key]['path'],self.profile['dacfunction'][key]['vref']) 
...
#        self.audio_fft_device=Axi4FftAnalyzer(self.profile["audio_dev_path"])
        ...
        self.wave_measure_device=Axi4TimeDetect_v1(self.profile["wave_measure"]["path"])    
        self.aid_device=dict()
        for  key in self.profile["AID"].keys():
            self.aid_device[key] = eval(self.profile['AID'][key]['ipcore'])(self.profile['AID'][key]['path'])           
        ...
        try:
            for key in self.profile['swdrstctrl'].keys():
                self.swd_device[key]=Axi4SwdCore(self.profile['swdrstctrl'][key])
        except:
...
#         return True
    ...
    def aid_data_config(self, channel,aid_index, aid_req_data, aid_resp_data):
        self.aid_device[channel].disable()
        self.aid_device[channel].enable()
        return self.aid_device[channel].cfg_aid_data(aid_index,aid_req_data,aid_resp_data)
    
    def aid_init(self,channel):
        self.aid_device[channel].disable()
        self.aid_device[channel].enable()
        return self.aid_device[channel].aid_slave_init()

    def aid_disable(self,channel):
        return self.aid_device[channel].disable()
    def swdrstctrl(self,channel,status):
        """swd rst pin ctrl       
            Args:
                channel("UUT1_MCU_RESET_L") 
            Returns:
                True| False       
        """        
        if channel not in  self.profile['swdrstctrl']:
            return False
        self.swd_device[channel].disable()
        self.swd_device[channel].enable()
        self.swd_device[channel].swd_rst_pin_ctrl(status)    
        return True
    ...
    def ad5761_write_register(self,addr,config_data):
        ret=self.dac_device["psu3_voltage"].write_register(addr,config_data)
        return True
...
#        self.ad7175_device.voltage_channel_enable(channel,self.profile["adc7175_config"][channel])
 #       logger.warning(str(self.profile["adc7175_config"][channel]))  
    ...
    def voltage_set(self,channel,voltage):
        ...
        """
        if channel !="base_board":
            self.gpio_device_name.gpio_set(self.profile["dacfunction"][channel]['gpio'])
        if channel=="psu3_voltage" or channel=="psu2_voltage":
            ret=self.dac_device[channel].output_voltage(voltage)
        else:
...
#         return ret
    ...
    def temperature_read(self):
        ...
        if self.temperature_device is None:
            return False
        self.temperature_device.disable()
        self.temperature_device.measure_disbale()
        self.temperature_device.enable()
        self.temperature_device.measure_enable()
        time.sleep(0.5)
        read_data = self.temperature_device.rd_data()

        temperature_value = (read_data[0] & 0xff) 
        temperature_value |= ((read_data[1] << 8) & 0xff00)

        if temperature_value & 0xf800 :
            temperature_value = ~temperature_value
            temperature_value += 1
            temperature_value &= 0xffff
            temperature_value = temperature_value * -0.0625
        else :
            temperature_value = temperature_value * 0.0625

        return temperature_value
    def mcu_io_set(self,mcu_gpio_list):
        """ set gpio of zynq 
            Args:          
            Returns:
                bool: True| False, False for mcu gpio set failed,True for mcu gpio set success
        """
        for key in mcu_gpio_list.keys():
            if key in self.profile["gpio_control"]:
                self.gpio_device_name.gpio_set([(self.profile["gpio_control"][key],mcu_gpio_list[key])])
            else:
                return False
        return True   
    @staticmethod
...
#            boards[board_name]['dacfunction'][key]["vref"] = board['dacfunction'][key]['vref']
            boards[board_name]['dacfunction'][key]['path']=utility.get_dev_path()  +'/' + board['dacfunction'][key]['path']
            boards[board_name]['dacfunction'][key]['gpio']=[(board['dacfunction'][key]['gpio1'],board['dacfunction'][key]['gpio1_state']),
...
#        boards[board_name]['audio_dev_path']= utility.get_dev_path()  +'/' +board['audiofunction']['audio_dev_path']
        ...
        for key in board['AID'].keys():
            boards[board_name]['AID'][key]["path"]=utility.get_dev_path()  +'/' +board['AID'][key]["path"]
...
#             
        if "temperature" in board:
            boards[board_name]['temperature'] = copy.deepcopy(board['temperature'])
            boards[board_name]['temperature']["path"]=utility.get_dev_path()  +'/' +board['temperature']["path"]
            
        boards[board_name]['swdrstctrl']=copy.deepcopy(board['swdrstctrl'])
        for key in board['swdrstctrl'].keys():
            boards[board_name]['swdrstctrl'][key]=utility.get_dev_path()  +'/' +board['swdrstctrl'][key]  
            
        boards[board_name]['gpio_control'] =copy.deepcopy(board['gpio_control'])        
