...
class AD7175(object):
    ...
    def is_comunication_ok(self):
        ...
        """
        id = self.register_read(0x07)
        if id is False:
            logger.warning("AD7175[%s]  read 0x07 reg  failed"%(self.device_path))
            return False

        if 0xcd0 != (id&0xfff0):
            logger.warning("AD7175[%s]  iD 0x%04x not match 0xcdX"%(self.device_path,id))
            return False
    ...
    def ifmode_register_set(self,data_state_channel_flag="enable",data_bits="24bits"):
        """ ADC IFMODE register set
            Args:
                ...
                data_bits: ["24bits","32bits"]
                    AD7177 sample code have 24 bits and 32 bits mode
            Returns:
                bool: True | False, True for success, False for failed.
